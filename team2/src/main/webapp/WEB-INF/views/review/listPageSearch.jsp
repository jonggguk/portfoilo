...
<table>
		...
		<c:forEach items="${list}" var="list">
			<tr>
				<td>${list.r_num}</td>
				<td>
					<a href="/team2/review/view?r_num=${list.r_num}">${list.r_title}</a>
				</td>
				<td>
...
<div>
	...
	<c:if test="${page.prev}">
		<span>[ <a href="/team2/review/listPageSearch?num=${page.startPageNum - 1}${page.searchTypeKeyword}">이전</a> ]</span>
	</c:if>
	...
	<c:forEach begin="${page.startPageNum}" end="${page.endPageNum}" var="num">
		<span>
			...
			<c:if test="${select != num}">
				<a href="/team2/review/listPageSearch?num=${num}${page.searchTypeKeyword}">${num}</a>
			</c:if> 			
	...
	<c:if test="${page.next}">
		<span>[ <a href="/team2/review/listPageSearch?num=${page.endPageNum + 1}${page.searchTypeKeyword}">다음</a> ]</span>
	</c:if>
	...
	<div>
		<select name="searchType">
	        ...
	        <option value="r_content" <c:if test="${page.searchType eq 'r_content'}">selected</c:if>>내용</option>
		    <option value="r_title_r_content" <c:if test="${page.searchType eq 'r_title_r_content'}">selected</c:if>>제목+내용</option>
		    <option value="u_id" <c:if test="${page.searchType eq 'u_id'}">selected</c:if>>작성자</option>
		</select>
...
<script>
	...
	document.getElementById("searchBtn").onclick = function () {
		...
		location.href = "/team2/review/listPageSearch?num=1" + "&searchType=" + searchType + "&keyword=" + keyword;
	};
</script>
</center>
<br><br>
